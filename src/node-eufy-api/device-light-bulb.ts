...
export class LightBulb extends AbstractDevice {
	...
	private async getState(): Promise<Message> {
		...
		if (!this.finishGetPreviousState) {
			return this.previousMessage;
		} else {
			this.finishGetPreviousState = false;
			setTimeout(() => {
				this.finishGetPreviousState = true;
			}, 10000);
			...
			const msg = await this.sendPacketWithResponse(packet);
			this.finishGetPreviousState = true;
			return msg;
		}
	}
	...
	private async setState(options: Partial<BulbState> & { power?: boolean }): Promise<void> {
		...
		if (this.finishSetPreviousState) {
			this.finishSetPreviousState = false;
			setTimeout(() => {
				this.finishSetPreviousState = true;
			}, 10000);
